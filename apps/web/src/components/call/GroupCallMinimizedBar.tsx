...
export function GroupCallMinimizedBar({ onLeave, onToggleMute, onExpand }: Props) {
  const active = useGroupCallStore((s) => s.active)
  if (!active) return null

  const total = active.participants.length + 1 // +1 for self
    ...
    <div className="fixed top-0 left-0 right-0 z-50 flex justify-center pointer-events-none">
      ...
      >
        ...
        <button onClick={onExpand} className="flex-1 min-w-0 text-left">
          <p className="text-white text-xs font-semibold leading-tight">Групповой звонок</p>
          <p className="text-green-400 text-[10px] leading-tight">{total} участника</p>
        </button>
